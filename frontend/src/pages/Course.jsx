...
export default function Course() {
  const { courses, loading, next, previous, goToNextPage, goToPreviousPage, keyword, setKeyword } = useCourses();
    ...
    <>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "20px" }}>
          {courses.map((course) => (
            <div key={course.id} className="card shadow-sm border-1 rounded h-100">
              <div className="card-body">
                <h5 className="card-title fw-bold text-primary">{course?.title}</h5>
                <p className="card-text text-muted">{course?.description}</p>
                <div className="d-flex align-items-center gap-2">
                  {getCurrentUserRole() === ROLES.STUDENT && (<button
                    type="button"
